...
public class GenericTree<T> implements Tree<T> {
    ...
    protected int height;           // keep track of the depth of the leaf so that we don't have to calculate it later 
    ...
    // will not work is the tree has a LOT of nodes
    @Override
    ...
    public TreeNode<T> search(T searchdata) {
        ...
        // if we make it here then the tree was not empty
    ...
    protected TreeNode<T> searchHelper(T searchdata, TreeNode<T> node) {
        if (node.element().equals(searchdata)) {
            return node; // we found it!!! return the recursive call hierarchy
        }
        if (node.isLeaf()) {
            return null;
        }
    ...
    protected TreeNode<T> searchHelperBFS(T searchdata, TreeNode<T> root) {
        if (root.element().equals(searchdata)) {
            return root; // we found it!!! return the recursive call hierarchy
        }
        if (root.isLeaf()) {
            return null;
        }
    ...
    protected TreeNode<T> searchLevel(T searchdata, TreeNode<T> children[]) {
        ...
        if (children == null) {
            return null; // we made it to the end and we never found it 
        }
        for (int i = 0; i < children.length; i++) {
            TreeNode<T> child = children[i];
